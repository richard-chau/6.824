...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if len(args.Entries) != 0 &&
		...
		rf.isConflict(args.Entries, rf.Slog[args.PrevLogIndex+1-rf.LastIncludedIndex:]) {
		rf.Slog = rf.Slog[:args.PrevLogIndex+1-rf.LastIncludedIndex] // not match with args.Entries[0]
	...
	if reply.Success == false { //handle log inconsistency.
		...
		if rf.OptimizeMultipleEntries != 1 {
			args := &AppendEntriesArgs{Term: rf.CurrentTerm,
				...
				Entries:      rf.Slog[rf.NextIndex[server]-rf.LastIncludedIndex:], //heartbeat empty
...
func (rf *Raft) GetLastLogIndex() int {
	return len(rf.Slog) - 1 + rf.LastIncludedIndex
}

func (rf *Raft) GetLastLogTerm() int {
	if len(rf.Slog)-1 == 0 && rf.LastIncludedIndex != 0 {
...
func (rf *Raft) GetPrevLogTerm(i int) int {
	...
	return rf.Slog[rf.NextIndex[i]-1-rf.LastIncludedIndex].Term ////~~~~
}
