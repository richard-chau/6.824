...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("Disconnected %d", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	DPrintf("Connected %d", leader1)
	cfg.connect(leader1)
	...
	DPrintf("Disconnected %d and %d", leader2, (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	DPrintf("Connected %d", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf("Connected %d", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		DPrintfB("This is index %d", index)
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintfB("Now leader: %d--------------------------- Disconnect %d", leader, (leader+1)%servers)
	...
	// re-connect
	DPrintfB("Now leader: %d--------------------------------- Reconnect %d", leader, (leader+1)%servers)
